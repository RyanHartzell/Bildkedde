...
def irrad2electrons(aperture_reffered_irrad, wl=550e-9, aperture=1.0, integration_time=1.0, transmission=1.0, qe=1.0, eod=1.0, shot=True):
    ...
    if shot==True:
        return xp.round(qe * eod * xp.random.poisson((aperture_reffered_irrad * aperture * integration_time * transmission) / photon_energy_per_wavelength(wl, mode="J")))
    else:
        return xp.round(qe * eod * (aperture_reffered_irrad * aperture * integration_time * transmission) / photon_energy_per_wavelength(wl, mode="J"))
...
def electrons2voltage(electrons, ve=1e-6, vv=1.0, ve_nonlinearity=0.0, vv_nonlinearity=0.0, offset=0.0):
    return electrons * ve * vv + offset
